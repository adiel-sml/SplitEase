...
export function ExpenseList({ group, onEditExpense, onDeleteExpense }: ExpenseListProps) {
  ...
  // Fallback devise du groupe
  const currencyCode = group.currency ?? 'EUR';
  const formatCurrency = (amount: number, currency?: string) =>
    currencyService.formatCurrency(amount, currency ?? currencyCode);
  ...
  const filteredExpenses = sortedExpenses.filter(expense => {
    ...
    const matchesSearch = !searchTerm || 
      ...
      getCategoryName(catId).toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {filteredExpenses.map((expense) => {
          const catId = expense.category || 'other';
          const displayCurrency = expense.currency ?? currencyCode; // fallback
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-lg">{getCategoryIcon(catId)}</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-600 dark:text-gray-400 mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span>{isNaN(dateObj.getTime()) ? '—' : dateObj.toLocaleDateString('fr-FR')}</span>
                </div>
                <div className="flex items-center gap-2">
